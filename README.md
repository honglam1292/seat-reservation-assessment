...
SQLite is used for local simplicity. For a production reservation system, PostgreSQL would be preferred because of stronger concurrency primitives, operational maturity, and better support for production workloads.
...
The migration adds a SQLite partial unique index so a seat can have only one active reservation:
...
- Prisma + SQLite: fast local setup with a clear schema and migration history.
